...
pub async fn ensure_vm_assets(config: &mut AppConfig) -> anyhow::Result<()> {
    ...
    if let Some(image_ref) = &config.firecracker.image {
        ...
        let cloud_img_url = config
            .firecracker
            .cloud_img_url
            .as_deref()
            .unwrap_or(DEFAULT_CLOUD_IMG_URL);
        ensure_golden_rootfs(
            &config.firecracker.rootfs_golden,
            cloud_img_url,
            &config.network,
        )
        .await?;
...
async fn ensure_directories(config: &AppConfig) -> anyhow::Result<()> {
    tokio::fs::create_dir_all(&config.runner.work_dir)
        .await
...
async fn resolve_allowed_networks(
...
fn preflight_kvm() -> anyhow::Result<()> {
    ...
    tracing::info!(
        virt_extensions = virt_count,
        "CPU virtualization support detected"
    );
...
async fn verify_kernel_overlay_support(kernel_path: &str) -> anyhow::Result<()> {
    ...
    let has_squashfs = data.windows(17).any(|w| w == b"squashfs: version");
    ...
    tracing::info!(
        path = kernel_path,
        "kernel verified: squashfs + overlay support present"
    );
    Ok(())
}

async fn ensure_golden_rootfs(
    rootfs_path: &str,
    cloud_img_url: &str,
    ...
    tracing::info!(
        path = rootfs_path,
        "golden rootfs not found, building from cloud image..."
    );
    ...
    let raw_img = format!("{}/cloud-raw.img", parent_str);
    {
        ...
        let mut output = tokio::fs::File::create(&raw_img)
            .await
    ...
    // Find ext4 partition and extract it (pure Rust — no losetup/blkid/dd)
    {
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            ...
            let mut img = std::fs::File::open(&raw_img_clone).context("opening raw image")?;
            ...
            let mut output = std::fs::File::create(&rootfs_out).context("creating rootfs file")?;
    ...
    let (block_count, block_size) =
        read_ext4_superblock(rootfs_path).context("reading ext4 superblock")?;
...
async fn build_rootfs_contents(mount_dir: &str, network: &NetworkConfig) -> anyhow::Result<()> {
    ...
    // Extract tar.gz in pure Rust
    {
        ...
        tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
            let file = std::fs::File::open(&tarball_path).context("opening runner tarball")?;
    ...
    tokio::fs::copy(&self_exe, &guest_bin)
        .await
...
"#,
    ...
    if !Path::new(&symlink_path).exists() {
        let _ = tokio::fs::symlink("/etc/systemd/system/rc-local.service", &symlink_path).await;
    }
...
async fn convert_to_squashfs(ext4_path: &str) -> anyhow::Result<()> {
    ...
    let status = tokio::process::Command::new("mount")
        ...
        status.success(),
        "mount ext4 for squashfs conversion failed"
    );
...
async fn ensure_nat_rules(network: &NetworkConfig) -> anyhow::Result<()> {
    ...
    if network.resolved_networks.is_empty() {
        ...
        tracing::info!(
            "network allowlist applied via ipset — unmatched outbound traffic will be dropped"
        );
    }
...
async fn add_iptables_rule_if_missing(
    add_args: &[&str],
    check_args: &[&str],
) -> anyhow::Result<()> {
...
fn read_ext4_superblock(path: &str) -> anyhow::Result<(u64, u64)> {
    ...
    let magic = u16::from_le_bytes([sb[56], sb[57]]);
        ...
        magic == 0xEF53,
        "not a valid ext4 image (magic: {:#06x})",
        magic
    );
