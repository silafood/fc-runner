...
pub async fn run(log_level: &str) -> anyhow::Result<()> {
    ...
    // Verify Podman (shimmed as docker) is accessible before starting the runner.
    // GitHub Actions services: containers need a working container runtime.
    verify_container_runtime().await;
...
/// Verify the container runtime (podman shimmed as docker) is working.
/// Podman is daemonless so no socket or service wait is needed — just check the binary.
async fn verify_container_runtime() {
    // Check `docker` (podman symlink) is available and works as the runner user
    let mut cmd = tokio::process::Command::new("docker");
    cmd.args(["info", "--format", "{{.Host.OCIRuntime.Name}}"])
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    #[cfg(unix)]
    ...
    match cmd.output().await {
        Ok(output) if output.status.success() => {
            let runtime = String::from_utf8_lossy(&output.stdout);
            tracing::info!(runtime = runtime.trim(), "container runtime ready (podman)");
        }
        Ok(output) => {
            let stderr = String::from_utf8_lossy(&output.stderr);
            tracing::warn!(stderr = stderr.trim(), "container runtime check failed");
            // Try podman directly as fallback diagnostic
                ...
                .await
            {
                ...
                tracing::info!(version = info.trim(), "podman binary available");
            }
        }
        Err(e) => {
            tracing::warn!(error = %e, "docker/podman command not found — services: containers will fail");
        }
    }
}
